...
(define (light=? a-value another-value)
  (if (and (light? a-value) (light? another-value))
      ...
      (cond
        [(light? a-value) (error "The second value is not a traffic")]
        [(light? another-value) (error "The first value is not a traffic light")]
        [else (error MESSAGE)])))

(check-error (light=? "red" "something else") "The second value is not a traffic")
(check-error (light=? "something else" "red") "The first value is not a traffic light")
(check-error (light=? "aha" "huh") MESSAGE)