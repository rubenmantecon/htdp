...
(check-expect (insert-everywhere/in-all-words "a" (list (list "d" "e") (list "e" "d")))
              (list (list "a" "d" "e")
                    (list "d" "a" "e")
                    (list "d" "e" "a")
                    (list "a" "e" "d")
...
(check-expect (insert-everywhere/in-one-word "a" '()) (list (list "a")))
(check-expect (insert-everywhere/in-one-word "a" (list "e")) (list (list "a" "e") (list "e" "a")))
(check-expect (insert-everywhere/in-one-word "a" (list "d" "e"))
              (list (list "a" "d" "e") (list "d" "a" "e") (list "d" "e" "a")))
...
(check-expect (words->strings (list (list "c" "a" "t") (list "b" "e" "a" "r"))) (list "cat" "bear"))
...
(check-expect (in-dictionary (list "cat" "bear" "cgxbljrcjkm")) (list "cat" "bear"))
...
(check-expect (exists-in-dict? "apple" (list "apple" "car")) #true)
(check-expect (exists-in-dict? "bkorceauh" (list "bear" "table")) #false)
(define (exists-in-dict? s dict)
  (cond
    [(empty? dict) #f]
    [else (or (string=? (first dict) s) (exists-in-dict? s (rest dict)))]))
