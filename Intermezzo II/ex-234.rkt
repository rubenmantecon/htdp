...
(define (make-row-ranking r)
  (list (make-cell-number (first r))
        (make-cell-string (second r))))
...
(define (make-row-number l)
  (cond
    [(empty? l) '()]
    [else (cons (make-cell-number (first l))
                (make-row-number (rest l)))]))
...
(define (make-row-string l)
  (cond
    [(empty? l) '()]
    [else (cons (make-cell-string (first l))
                (make-row-string (rest l)))]))
...
(define (make-table-number row1 row2)
  `(table ((border "1"))
          (tr ,@(make-row-number row1))
          (tr ,@(make-row-number row2))))

; List-of-numbers List-of-numbers -> ... nested list ...
; creates an HTML table from two lists of numbers 
(define (make-table-string row1 row2)
  `(table ((border "1"))
          (tr ,@(make-row-string row1))
          (tr ,@(make-row-string row2))))
...
; TODO: crea funciones universales para cells, rows y tables que permitan componer Ranks